...
export function VerticalLinearStepper(props) {
 ...
 const [activeProgress, setActiveProgress] = React.useState(2);
 ...
 React.useEffect(() => {
  ...
  const handleNext = () => {
    ...
    setTimeout(function () {
      window.location.href = '/courses/101/iii';
    }, 500);
...
class Introii extends Component {
  constructor(props) {
    super(props);
    this.state = {
      progress: 0,
...
Introii = connect(mapStateToProps, { clearErrors })(Introii);

export default Introii = reduxForm({
  form: '',
  fields: [''],
  clearErrors,
