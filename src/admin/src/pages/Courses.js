...
const TABLE_HEAD = [
  ...
  { id: 'link', label: 'URL', alignRight: false },
...
export default function UserPage() {
  ...
  useEffect(() => {
    const fetchCertificates = async () => {
      try {
        const response = await axios.get('https://edunode.herokuapp.com/api/addedcours/cours');
  ...
  const handleEditCertificate = async (certificateId) => {
    ...
    try {
      ...
      if (newStatus !== null) {
        await axios.put(`https://edunode.herokuapp.com/api/addedcours/edit/${certificateId}`, {
          status: newStatus,
        });
  ...
  const handleDeleteCertificate = async (certificateId) => {
    try {
      ...
      if (confirmed) {
        await axios.delete(`https://edunode.herokuapp.com/api/addedcours/delete/${certificateId}`);
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((certificate) => {
                    const { _id, title, description ,status, email,link,questions } = certificate;
